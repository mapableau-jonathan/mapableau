...
export type Provider = {
  ...
  serviceAreas: string[];
  specialisations: { specialisationDefinition: { id: string; name: string } }[];
  services: { serviceDefinition: { id: string; name: string } }[];
  businessHours: {
    id: string;
    dayOfWeek: DayOfWeek;
    openTime: Date;
...
export type ProviderOutlet = {
  id: string;
  providerId: string;
  provider: Omit<Provider, "specialisations" | "services" | "businessHours">;
  name: string;
  rating: number | null;
  reviewCount: number;
  logoUrl: string | null;
...
export type SelectedProviderOrOutlet =
  | {
      type: "provider";
      provider: Provider;
      distanceKm: number;
