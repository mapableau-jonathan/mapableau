...
import { parseBody, registerProviderSchema } from "@/lib/validation";

export async function POST(req: Request) {
  const [body, err] = await parseBody(req, registerProviderSchema);
  if (err) return err;

  const { email, password, name, suburb, state, postcode } = body;
