/**
 * Google OAuth Callback via Passport
 * Handles Google OAuth callback and creates NextAuth session
 */
...
export async function GET(request: NextRequest) {
  return new Promise((resolve) => {
    passport.authenticate("google", async (err: Error | null, profile: any) => {
      if (err || !profile?.id) {
      ...
      try {
        await findOrCreateUserFromPassport("google", profile.id, {
          ...
          expiresAt: profile.expires_in
            ? Math.floor(Date.now() / 1000) + profile.expires_in
            : undefined,
        });
        ...
        resolve(
          NextResponse.redirect(
            new URL(
              `/api/auth/callback/google?callbackUrl=${encodeURIComponent(callbackUrl)}`,
              request.url
            )
          )
        );
